...
- **Everything installed came from your registry, transitively.** `--registry`
  only routes the specs the runner passes; a package on the registry is free to
  declare a dependency as a git or tarball URL, and npm would fetch it. npm has
  no setting that forbids that, so the resolved tree is verified afterwards
  instead — every package must record a source under
  `CODEAPI_DEPENDENCY_NPM_REGISTRY`, or the job fails before it runs. Combined
  with `--ignore-scripts`, nothing from an unapproved host has executed.
