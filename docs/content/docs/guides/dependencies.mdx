...
- **The result is read-only to the sandbox.** The per-job directory is mounted
  read-only on `PYTHONPATH` and `NODE_PATH`. It is executable — unlike
  `/mnt/data` — which is why compiled extensions and native npm binaries load
  from it and not from anything the sandbox can write.
