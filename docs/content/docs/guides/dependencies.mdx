...
`# requirements are listed in README` declares nothing. Neither does a line
inside a string: a docstring or template literal that quotes
`# requirements: cowsay==6.1` as example text is skipped, so a program that
prints its own usage does not install what its usage describes.
...
- **Everything installed came from your index, transitively.** `--index-url` and
  ...
  setting that forbids it, so the installers run behind a proxy that resolves
  only the index, the registry and `CODEAPI_DEPENDENCY_ALLOWED_HOSTS` — the
  request fails at the proxy rather than being caught after the runner has
  already made it. The resolved tree is still verified afterwards, npm against
  the lockfile and pip against its `--report`, which catches a direct URL that
  happened to point at an allowed host. The npm check honours a path-scoped
  registry, so a different repository on the same host is refused too. Combined
  with `--ignore-scripts` and wheels-only, nothing from an unapproved host has
  executed.
- **The install cannot fill the runner's disk.** `CODEAPI_DEPENDENCY_MAX_BYTES`
  is enforced while the installer runs, not only once it finishes, so a package
  that expands to far more than the ceiling is killed on the way past it rather
  than after the space is gone.
...
| `CODEAPI_DEPENDENCY_ALLOW_PRIVATE_INDEX` | `false` | Allow an index on a private address |

`CODEAPI_DEPENDENCY_ALLOWED_HOSTS` exists because an index and its files often
live on different hosts — PyPI lists on `pypi.org` and delivers from
`files.pythonhosted.org`. An index that serves its own files needs nothing here.
An internal mirror additionally needs `CODEAPI_DEPENDENCY_ALLOW_PRIVATE_INDEX`,
since the installers' proxy refuses private addresses the same way the sandbox's
does.
