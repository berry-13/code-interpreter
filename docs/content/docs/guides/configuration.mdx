...
| `MAX_RUN_TIMEOUT` | `JOB_TIMEOUT` | Ceiling for a request's own `run_timeout` (ms). |
...
only narrow the deployment's ceiling, never raise it.
...
kills the run, the worker waits one grace longer than that, and the API waits
one grace longer than the worker. Each layer therefore outlives the one it is
waiting on, so the structured timeout above propagates to the caller instead of
racing an outer give-up and surfacing as a generic 500. Raise `JOB_TIMEOUT` and
