...
| `EXECUTION_MANIFEST_MAX_UPLOAD_BYTES` | — | Upload budget granted to one execution. |
| `EXECUTION_MANIFEST_MAX_OUTPUT_FILES` | — | Output file count budget. |
| `EXECUTION_MANIFEST_MAX_REQUESTS` | — | Total gateway requests one execution may make. |
...
| `SANDBOX_RUN_TIMEOUT` | `300000` | Run wall-clock limit (ms). |
...
| `SANDBOX_FORWARD_TARGET` | — | Where that port forwards to. |
