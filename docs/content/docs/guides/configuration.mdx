...
the job up. On a saturated queue the API's wait can therefore expire before the
job finishes, and what the caller should do about it depends on whether it ever
started:
...
| `CODEAPI_DEPENDENCY_ALLOWED_HOSTS` | `files.pythonhosted.org` | Other hosts the installers may reach. |
| `CODEAPI_DEPENDENCY_ALLOW_PRIVATE_INDEX` | `false` | Allow an index or registry on a private address. |
| `CODEAPI_DEPENDENCY_MAX_COUNT` | `50` | Max packages per job. |
| `CODEAPI_DEPENDENCY_REQUIRE_PINNED` | `true` | Require `name==version`. Set false to accept bare names and ranges. |
...
Off by default. See [Sandbox networking](/docs/guides/networking).

| Variable | Default | Description |
| --- | --- | --- |
| `CODEAPI_ALLOW_SANDBOX_NETWORK` | `false` | Allow user code outbound HTTP/HTTPS through a per-job proxy. |
...
Non-public destinations (private, loopback, link-local/metadata, CGNAT, ULA)
are refused unconditionally and cannot be enabled by configuration.
